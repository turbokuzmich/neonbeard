...
import { getCatalogItems } from "./old";
...
export class Session extends Model {
  ...
  async getCartWeight() {
    ...
    return cartItems.reduce((total, item) => {
      const uid = getUniversalId({ id: item.itemId, variant: item.variantId });
      const weight = universal[uid].weight;

      return total + item.qty * weight;
    }, 0);
  }
...
Order.init(
  {
    ...
    address: {
      type: DataTypes.VIRTUAL,
      get() {
        if (this.type === DeliveryType.cdek) {
          return this.cdekPointAddress;
        }
